...
public class MultiReaderLock {
	...
	private class ReadLock implements SimpleLock {
		...
		public void unlock() throws IllegalStateException {
			synchronized (lock) {
				if (readers <= 0) {
					throw new IllegalStateException("o readers unlock.");
				}
				readers--;
				if (readers == 0) {
					lock.notifyAll();
	...
	private class WriteLock implements SimpleLock {
		...
		public void lock() {
			synchronized (lock) {
				try {
					while ((writers > 0 && !isActiveWriter()) || (readers > 0 && !isActiveWriter())) {
						lock.wait();
					}
					writers++;
		...
		public void unlock() throws IllegalStateException, ConcurrentModificationException {
			synchronized (lock) {
				...
				if (activeWriter != Thread.currentThread()) {
					throw new ConcurrentModificationException("Only the active writer can unlock the write lock.");
				}
				writers--;
				if (writers == 0) {
					activeWriter = null;
